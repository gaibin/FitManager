  ...
  return (
    <div className="space-y-5 animate-in">
      ...
      {generating && (
        <div className="bg-white rounded-2xl p-4 border border-gray-100" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-400">{lang === 'zh' ? '正在生成 PDF...' : 'Generating PDF...'}</span>
            <span className="text-xs font-bold text-[#007AFF] font-mono">{progress.current}/{progress.total}</span>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
            <div className="h-full rounded-full transition-all duration-300" style={{ width: `${(progress.current / progress.total) * 100}%`, background: 'linear-gradient(90deg, #007AFF, #5856D6)' }} />
          </div>
        </div>
      )}

      {error && (
        <div className="bg-[#FF3B30]/5 border border-[#FF3B30]/10 rounded-2xl p-4 text-sm text-[#FF3B30] font-medium">{error}</div>
      )}

      {/* 2x2 Preview Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <PreviewCard label={lang === 'zh' ? '第 1 页 · 封面' : 'Page 1 · Cover'}>
          <CoverPage member={member} lang={lang} studioName={studioName} />
        </PreviewCard>
        <PreviewCard label={hasAssessment ? (lang === 'zh' ? '第 2 页 · 体态评估' : 'Page 2 · Posture') : (lang === 'zh' ? '第 2 页 · 训练记录' : 'Page 2 · History')}>
          {hasAssessment ? <PosturePage member={member} lang={lang} studioName={studioName} /> : <HistoryPage member={member} lang={lang} studioName={studioName} />}
        </PreviewCard>
        {hasAssessment && (
          <>
            <PreviewCard label={lang === 'zh' ? '第 3 页 · 矫正方案' : 'Page 3 · Plan'}>
              <PlanPage member={member} lang={lang} studioName={studioName} />
            </PreviewCard>
            <PreviewCard label={lang === 'zh' ? '第 4 页 · 训练记录' : 'Page 4 · History'}>
      ...
      <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
        <div ref={coverRef}><CoverPage member={member} lang={lang} studioName={studioName} /></div>
        {hasAssessment && (<>
